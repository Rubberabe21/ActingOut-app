...
ASSETS.COVER.src = 'assets/copertina.png';
ASSETS.LAURA.src = 'assets/laura.png';
ASSETS.GUIDO.src = 'assets/guido.png';
ASSETS.MOV_LAURA.src = 'assets/mov-laura.png';
ASSETS.MOV_GUIDO.src = 'assets/mov-guido.png';

['red', 'yellow', 'blue', 'green'].forEach(color => {
  const lImg = new Image(); lImg.src = `assets/laura ${color}.png`;
  ASSETS.VS_SPRITES.LAURA[color] = lImg;
  const gImg = new Image(); gImg.src = `assets/guido ${color}.png`;
  ASSETS.VS_SPRITES.GUIDO[color] = gImg;
});

ASSETS.BOMBA.src = 'assets/bomba.png';
ASSETS.FUOCO.src = 'assets/fuoco.png';
ASSETS.SCARPA.src = 'assets/scarpa.png';
ASSETS.CAFFE.src = 'assets/caffe.png';
ASSETS.CUORE.src = 'assets/cuore.png';

for (let i = 1; i <= 5; i++) {
  const imgM = new Image(); imgM.src = `assets/mostro${i}.png`; ASSETS.MOSTRI[i] = imgM;
  const imgB = new Image(); imgB.src = `assets/boss${i}.png`; ASSETS.BOSSI[i] = imgB;
}
...
const ERAS_INFO = [
  { era: 1, name: "ERA 1: GLITCH .GIF", monster: "Glitch .GIF", boss: "MEGA GIF CORROTTO", colBase: "#ff007f", colTough: "#ffaa00", filterTough: "hue-rotate(60deg) saturate(200%)", storyEra: ["I vecchi raster animati sono impazziti!", "I file .GIF corrotti mutano nel server.", "Pulisci la cartella prima della consegna!"], storyBoss: ["Un Loop Infinito blocca il rendering!", "La bozza gigante resiste a 8 colpi.", "Patrolling lineare continuo ad alta stabilità!"] },
...
const STORY_DROP_GUARANTEE_AFTER = 4;
...
function loadLevel(options = {}) {
  ...
  player.x = 1.5 * CELL_SIZE; player.y = 1.5 * CELL_SIZE; player.invTimer = 90; player.isMoving = false; player.facingLeft = false;
  ...
  for (let r = 0; r < ROWS; r++) {
    ...
    for (let c = 0; c < COLS; c++) {
      if (r === 0 || r === ROWS - 1 || c === 0 || c === COLS - 1 || (!isBossLevel && r % 2 === 0 && c % 2 === 0)) {
        ...
        let spawnChance = isBossLevel ? 0.22 : (0.42 + Math.min(level * 0.004, 0.14));
  ...
  if (isBossLevel) {
    let bossEra = Math.min(5, Math.floor(level / 10)), bossHp = 8 + (bossEra - 1) * 3;
    for (let br = 5; br <= 8; br++) for (let bc = 5; bc <= 8; bc++) { map[br][bc] = 0; blockData[br][bc] = null; }
    enemies.push({ isBoss: true, era: bossEra, name: ERAS_INFO[bossEra - 1].boss, gx: 5, gy: 6, x: 6 * CELL_SIZE, y: 7 * CELL_SIZE, dirX: 1, dirY: 0, hp: bossHp, maxHp: bossHp, speed: 0.78 + bossEra * 0.08, hitTimer: 0, burstTimer: 0 });
  } else {
    let emptyCells = [];
    for (let r = 1; r < ROWS - 1; r++) for (let c = 1; c < COLS - 1; c++) if (map[r][c] === 0 && !(r <= 3 && c <= 3)) emptyCells.push({ r, c });
    let enemyCount = Math.min(2 + Math.floor(((level - 1) % 10) / 3) + Math.floor((level - 1) / 20), 6);
    for (let i = 0; i < enemyCount; i++) {
      ...
      // Dal livello 3 almeno un nemico corazzato è sempre presente: non dipende più dalla fortuna.
      let isGuaranteedArmored = level >= 3 && i === 0;
      let enemyHp = (isGuaranteedArmored || Math.random() < (0.08 + Math.min(level * 0.01, 0.30))) ? 2 : 1;
      enemies.push({ isBoss: false, era: mType, gx: ec, gy: er, x: ec * CELL_SIZE + CELL_SIZE / 2, y: er * CELL_SIZE + CELL_SIZE / 2, dirX: Math.random() < 0.5 ? 1 : -1, dirY: 0, hp: enemyHp, maxHp: enemyHp, hitTimer: 0, speed: 0.82 + mType * 0.08 + Math.min(level * 0.006, 0.28) });
    }
  }
...
function placeBomb() {
  ...
  bombs.push({ id: Date.now() + Math.random(), gx, gy, timer: 112, range: player.bombRange, exploded: false });
  playSound('bomb');
}
...
function updateBombsAndExplosions() {
  ...
  if (enemies.length === 0 && state === 'playing') {
    ...
    if (level > 50) { deleteStorySave(); state = 'gameover'; saveScore(score); ArcadeGameShell.showEndActions({ onRetry: () => { state = 'CHAR_SELECT'; }, onExit: () => exitGame(score) }); }
    else {
...
function hitPlayer() {
  ...
  lives--; player.invTimer = 120; triggerShake(15, 8); playSound('fail');
  if (lives <= 0) { deleteStorySave(); state = 'gameover'; saveScore(score); ArcadeGameShell.showEndActions({ onRetry: () => { state = 'CHAR_SELECT'; }, onExit: () => exitGame(score) }); }
...
function drawSingleEnemy(e) {
  ...
  if (e.isBoss) {
    ...
    if (e.maxHp > 1) {
      // Cornice sempre visibile anche sui browser che rendono poco evidente il filtro colore.
      ctx.strokeStyle = eraData.colTough;
      ctx.lineWidth = 2;
      ctx.strokeRect(ex - r - 3, ey - r - 3, r * 2 + 6, r * 2 + 6);
...
function drawScreens() {
  ...
  if (state === 'ERA_INTRO') {
    ...
    if (isBossLvl) {
      ...
      drawCrispText(`RESISTENZA: ${8 + (bossEra - 1) * 3} COLPI`, W / 2, 254, 'bold 11px Courier New', '#ffffff');
    } else {
