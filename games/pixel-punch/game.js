...
(() => {
  ...
  class SelectScene extends Phaser.Scene {
    ...
    preload() {
      ...
      this.load.image('cover', 'assets/copertina.png');
    ...
    handleAction(action) { 
      ...
      else if (action === 'exit') window.location.href = '../../hub/index.html';
  ...
  class ScenarioIntroScene extends Phaser.Scene {
    ...
    create() {
      ...
      this.actionHandler = event => {
        ...
        else if (event.detail === 'exit') window.location.href = '../../hub/index.html';
  ...
  class GameScene extends Phaser.Scene {
    ...
    exitToHub() {
      this.stopGuard();
      this.saveScore().finally(() => { window.location.href = '../../hub/index.html'; });
    }
    ...
    showFinalVictory() {
      ...
      const button = this.add.text(W / 2, 390, 'TORNA ALLA SELEZIONE', { fontFamily: 'monospace', fontSize: '14px', backgroundColor: '#7028aa', padding: { x: 15, y: 11 } }).setOrigin(0.5).setScrollFactor(0).setDepth(13001).setVisible(false);
      ArcadeGameShell.showEndActions({ onRetry: () => this.scene.start('SelectScene'), onExit: () => this.exitToHub() });
    ...
    gameOver() {
      ...
      const retry = this.add.text(W / 2, 380, 'RIPROVA', { fontFamily: 'monospace', fontSize: '20px', color: '#ffffff', backgroundColor: '#c71950', padding: { x: 32, y: 13 } }).setOrigin(0.5).setScrollFactor(0).setDepth(12502).setVisible(false);
      retry.on('pointerdown', () => this.scene.restart({
        character: this.character, stage: this.stage, score: this.score,
        stagePower: this.stagePower, shopPowerKeys: this.shopPowerKeys
      }));
      ArcadeGameShell.showEndActions({
        onRetry: () => this.scene.restart({ character: this.character, stage: this.stage, score: this.score, stagePower: this.stagePower, shopPowerKeys: this.shopPowerKeys }),
