const AudioManager = (function () {
  ...
  const audioMap = {
    hub: 'music_hub.mp3',
    'deadline-drive': 'music_deadline_drive.mp3',
    'file-bomber': 'music_file_bomber.mp3',
    'feedback-invaders': 'music_invaders.mp3',
    'pixel-punch': 'music_punch.mp3'
  };
  ...
  async function loadBGM() {
    ...
    const section = Object.keys(audioMap).find(key => pathParts.includes(key)) || 'hub';
    const bgmFile = audioMap[section];

    if (!bgmFile) return;

    try {
      const res = await fetch(new URL(`assets/audio/${bgmFile}`, appRoot));
