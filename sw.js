const CACHE='acting-arcade-shell-v1';
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(['./start.html','./start.css','./start.js','./manifest.json','./assets/icon.png','./assets/hub/logo.png'])).then(()=>self.skipWaiting())));
